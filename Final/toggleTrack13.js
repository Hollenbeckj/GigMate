...
//objects
var bank13;//patcher that will be assigned to the bpatcher object (make sure it is in the same folder as the script it is assigned to)
...
{
	var button = this.patcher.getnamed("button13");//search for an object named button 1 in the main patcher and assign it to the variable button (you can name objects using the inspector in max)
	...
	if(msg = "jsobject 0") //if a message from the terminal is recieved and it sais jsobject 0 
	{
		...
		bank13 = this.patcher.newdefault(300, 100, "bpatcher");//set the variable bank1 to be a bpatcher that is located at 300 on the x axis and 100 on the y axis 
		bank13.patching_rect(1574,400,700,165);//set the patching rect of the the b patcherto be at location x 174 and y location 70, the size of the b patcher will be 450 pixels wide and 165 pixels long 
		bank13.name('Track13');//set the patcher bank1 to be dynamically assinged to the bpatcher made outside the program and then create the bpatcher with the assignments 
		//buttonPushed += 1;
	...
	if(msg = "jsobject 0") // check again if a message has been received from the button
	{
		...
		{
			this.patcher.remove(bank13);//delete the already created bank titled bank1
